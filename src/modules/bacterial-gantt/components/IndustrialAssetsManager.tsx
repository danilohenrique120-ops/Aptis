...
function IndustrialAssetsManager({
  ...
  const [customScaleLabels, setCustomScaleLabels] = useState<Record<ScaleType, string>>(() => {
    const saved = getSafeStorage('pcp_custom_scale_labels');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
  ...
  const handleSaveTabLabel = (scale: ScaleType) => {
    ...
    setSafeStorage('pcp_custom_scale_labels', JSON.stringify(updated));
  ...
  const handleCreateNewScaleTab = (e: React.FormEvent) => {
    ...
    if (!customScalesList.includes(key)) {
      ...
      setSafeStorage('pcp_custom_scales_list', JSON.stringify(updatedScales));
    }

    const updatedLabels = { ...customScaleLabels, [key]: nameClean };
    setCustomScaleLabels(updatedLabels);
    setSafeStorage('pcp_custom_scale_labels', JSON.stringify(updatedLabels));
  ...
  const handleDeleteCustomScaleTab = (scale: ScaleType) => {
    if (confirm(`Deseja excluir a escala "${customScaleLabels[scale] || scale}"?`)) {
      ...
      setSafeStorage('pcp_custom_scale_labels', JSON.stringify(copyLabels));
  ...
  const handleMoveScaleTab = (scale: ScaleType, direction: 'left' | 'right') => {
    ...
    setSafeStorage('pcp_custom_scales_list', JSON.stringify(reorderedAll));
  };
