...
function GanttTimeline({ batches, preventatives, recipes, onDeleteBatch, onDeletePreventative, onUpdateBatches, onAddDeviationLog, setupTimes, envaseLinesCount, scaleCounts, onUpdateScaleCount, customAssets, deviations = [], planningErrors = [], onDeleteCampaignBatches }: GanttTimelineProps) {
  const [visibleScales, setVisibleScales] = useState<Record<ScaleType, boolean>>(() => {
    const saved = getSafeStorage('pcp_gantt_visible_scales');
    if (saved) {
      try {
  ...
  useEffect(() => {
    setSafeStorage('pcp_gantt_visible_scales', JSON.stringify(visibleScales));
  }, [visibleScales]);
  ...
  const [activeYear, setActiveYear] = useState<number>(() => {
    const saved = getSafeStorage('pcp_gantt_active_year');
    if (saved !== null) {
  ...
  useEffect(() => {
    setSafeStorage('pcp_gantt_active_year', String(activeYear));
  }, [activeYear]);

  const [activeMonth, setActiveMonth] = useState<number | null>(() => {
    const saved = getSafeStorage('pcp_gantt_active_month');
    if (saved !== null) {
  ...
  useEffect(() => {
    if (activeMonth === null) {
      setSafeStorage('pcp_gantt_active_month', 'all');
    } else {
      setSafeStorage('pcp_gantt_active_month', String(activeMonth));
    }
  }, [activeMonth]);
