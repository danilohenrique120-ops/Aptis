...
export default function GanttTimeline({ batches, preventatives, recipes, onDeleteBatch, onDeletePreventative, onUpdateBatches, onAddDeviationLog, setupTimes, envaseLinesCount }: GanttTimelineProps) {
  ...
  // Determine if today's date context (June 5th, 2026 as per metadata) is visible
  const virtualTodayStr = '2026-06-05T17:32:08Z';
  const todayDate = new Date(virtualTodayStr);
  const isTodayVisible = todayDate >= timelineStart && todayDate <= timelineEnd;
  
  // Calculate today line horizontal position
  let todayLinePos = 0;
  if (isTodayVisible) {
    const diffHours = (todayDate.getTime() - timelineStart.getTime()) / (1000 * 60 * 60);
    ...
    <div className="space-y-4" id="gantt-root">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          <div className="bg-slate-900 text-white rounded-lg p-2 shrink-0">
            <Calendar size={18} />
          </div>
          <div>
            <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Cronograma de Produção Avançado</h2>
            <p className="text-[11px] font-mono font-medium text-slate-500">
              Visualização de {formatShortDate(timelineStart.toISOString())} a {formatShortDate(timelineEnd.toISOString())}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 border-r border-slate-200 pr-2 mr-1">
            <span className="text-[10px] font-bold text-slate-400 uppercase mr-1">Densidade:</span>
            <button
              onClick={() => setZoomLevel(96)}
              className={`px-1.5 py-1 text-[10px] font-black rounded ${zoomLevel === 96 ? 'bg-slate-100 text-slate-800 border border-slate-350' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Alta
            </button>
            <button
              onClick={() => setZoomLevel(120)}
              className={`px-1.5 py-1 text-[10px] font-black rounded ${zoomLevel === 120 ? 'bg-slate-100 text-slate-800 border border-slate-350' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Média
            </button>
            <button
              onClick={() => setZoomLevel(160)}
              className={`px-1.5 py-1 text-[10px] font-black rounded ${zoomLevel === 160 ? 'bg-slate-100 text-slate-800 border border-slate-350' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Ampla
            </button>
          </div>
          ...
          <button
            ...
            className="p-1.5 bg-white border border-slate-250 text-slate-700 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer"
          ...
          <button
            ...
            className="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-250 rounded-lg text-xs font-semibold font-mono transition-colors cursor-pointer"
            title="Ir para Junho 2026"
          >
            Junho 2026 (Início)
          </button>
          ...
          <select
            ...
            className="px-2 py-1.5 border border-slate-250 rounded-lg bg-white text-xs font-semibold text-slate-700 focus:outline-none"
          >
            <option value={14}>Visualizar 2 Semanas (14 dias)</option>
            <option value={21}>Visualizar 3 Semanas (21 dias)</option>
            <option value={28}>Visualizar 4 Semanas (28 dias)</option>
          </select>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col" id="gantt-board">
        ...
        <div className="flex bg-slate-50 text-xs border-b border-slate-200 px-4 py-2 font-semibold text-slate-500 justify-between items-center select-none">
          ...
          <div className="text-[10px] text-indigo-600 bg-indigo-50 font-bold px-2 py-0.5 rounded flex items-center gap-1">
            <span className="animate-ping w-1.5 h-1.5 rounded-full bg-indigo-500"></span> Linha vermelha representa o dia virtual atual (05/06/2026 PCP)
          </div>
        </div>
              ...
              <div className="flex h-9 bg-slate-800 text-white divide-x divide-slate-700">
                {daysArray.map((day, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`h-full flex flex-col justify-center items-center text-[10px] font-medium leading-tight shrink-0 ${bgClass}`}
            ...
            >
              ...
              <div className="absolute inset-y-0 left-0 flex pointer-events-none z-0">
                {daysArray.map((day, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`h-full border-r border-slate-250/50 shrink-0 ${
              ...
              {isTodayVisible && (
                <div 
                  ...
                  title={`Data virtual atual PCP: ${formatFullDate(virtualTodayStr)}`}
                >
                  <div className="absolute top-0 -translate-x-1/2 bg-red-500 text-white px-1.5 py-0.5 rounded text-[8px] font-black tracking-widest uppercase shadow-md leading-none">
                    AGORA (05/06)
                  </div>
                </div>
              )}
